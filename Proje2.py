...
#Proje1 den farklı olarak Hx Radiothreapy ve M verisini çıkarıyoruz çünkü bu verilerde seçicilik yok bütün seçeneklerde veriler aynı 
#bir de yaş verilerini StandartScaler kullanarak scale ediyoruz
...
def submit_form():
    ...
    # Hx Radothyreapy ve M verisini çıkarıyoruz
    data.pop("Hx Radiothreapy")
    data.pop("M")
    

    #Verileri Scale edince model yaş verisine duyarlı hale geliyor
    scaler = StandardScaler()
    yas = data["Age"]
    yas_reshaped = yas.values.reshape(-1, 1)
    age_scaled = scaler.fit_transform(yas_reshaped)
    data["Age"] = age_scaled
    print(age_scaled)
    ...
    columns_to_encode = ['Gender', 'Smoking', 'Hx Smoking',
        'Thyroid Function', 'Physical Examination', 'Adenopathy', 'Pathology',
        'Focality', 'Risk', 'T', 'N', 'Stage', 'Response']
...
troid_location = ttk.Combobox(root, values=["Single nodular goiter-left", "Multinodular goiter", "Single nodular goiter-right", "Normal", "Diffuse goiter"])
